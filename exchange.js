// =====================================================================
// TG-SHI v5.2 — js/exchange.js
// Exchange hours tracking — multi-partner, per-owner balances
// =====================================================================
...
const Exchange = (() => {
  ...
  function renderDashboardWidget() {
    ...
    partners.forEach(p => {
      ...
      html += `<div class="qr"><div class="ql">${p.name} <span style="font-size:9px;color:#8892A4">(${p.partner_plane})</span></div><div class="qv" style="font-size:10px">
  ...
  // Positive = we have credits ON their plane, negative = we owe them hours on ours
  function calcBalance(entries, owner, rate) {
    let balance = 0;
    entries.filter(e => e.paid_by === owner).forEach(e => {
      if (e.direction === 'given') {
        // We gave them hours on OUR plane → we earn credits on THEIR plane
        balance += e.hours * rate;
      } else if (e.direction === 'received') {
        // We used hours on THEIR plane → spend our credits
        balance -= e.hours;
      }
    });
    return balance;
  ...
  function buildExchangePage() {
    ...
    if (partners.length > 0) {
      ...
      partners.forEach(p => {
        ...
        balanceHtml += `<div class="card"><div class="ch"><div class="ct">✈ ${p.name} — ${p.partner_plane}</div><div style="font-size:9px;color:#8892A4">Rate ${p.exchange_rate}:1</div></div><div class="cb">
          <div class="qr"><div class="ql">Hrs dadas (en TG-SHI)</div><div class="qv">${givenH.toFixed(1)}</div></div>
          <div class="qr"><div class="ql">Hrs recibidas (en ${p.partner_plane})</div><div class="qv">${recvH.toFixed(1)}</div></div>
          <div class="qr"><div class="ql">Balance COCO</div><div class="qv ${balCOCO >= 0 ? 'c2' : 'c3'}">${balCOCO >= 0 ? '+' : ''}${balCOCO.toFixed(1)} hrs en ${p.partner_plane}</div></div>
          <div class="qr"><div class="ql">Balance CUCO</div><div class="qv ${balCUCO >= 0 ? 'c2' : 'c3'}">${balCUCO >= 0 ? '+' : ''}${balCUCO.toFixed(1)} hrs en ${p.partner_plane}</div></div>
        </div></div>`;
      });
      balanceHtml += '</div>';
    } else {
    ...
    container.innerHTML = filtered.slice(0, 50).map(e => {
      ...
      return `<div class="fi">
        ...
        <div class="fm">
          ...
          <div class="fme"><span>${pName}</span><span>💳 ${e.paid_by}</span>${e.fuel_cost ? `<span>⛽ Q${e.fuel_cost}</span>` : ''}${e.pilot_cost ? `<span>🧑‍✈️ $${e.pilot_cost}</span>` : ''}${e.notes ? `<span>${e.notes}</span>` : ''}</div>
        </div>
  ...
  function openNewExchange() {
    ...
    let partnerOpts = partners.map(p => `<option value="${p.id}">${p.name} (${p.partner_plane})</option>`).join('');
    ...
    document.getElementById('book-form').innerHTML = `
      <div class="fs"><label class="fl">Socio</label><select id="xf-partner">${partnerOpts}</select></div>
      <div class="fs"><label class="fl">Dirección</label>
        <div class="tg">
          <div class="tc on" data-t="given" onclick="Exchange.xfDir('given',this)"><div class="ti">→</div><div class="tn">Dado</div><div class="td">Ellos volaron en TG-SHI</div></div>
          <div class="tc" data-t="received" onclick="Exchange.xfDir('received',this)"><div class="ti">←</div><div class="tn">Recibido</div><div class="td">Nosotros volamos en su avión</div></div>
        </div>
      </div>
  ...
  function openAddPartner() {
    ...
    document.getElementById('edit-form-content').innerHTML = `
      ...
      <div class="fs"><label class="fl">Matrícula del avión del socio</label><input type="text" id="xp-plane" placeholder="ej. TG-ABC" oninput="this.value=this.value.toUpperCase()"></div>
  ...
  function editPartner(id) {
    ...
    document.getElementById('edit-form-content').innerHTML = `
      ...
      <div class="fs"><label class="fl">Matrícula</label><input type="text" id="xpe-plane" value="${p.partner_plane}" oninput="this.value=this.value.toUpperCase()"></div>
  ...
  async function savePartner(id) {
    ...
    p.partner_plane = document.getElementById('xpe-plane').value.trim().toUpperCase();
  ...
  return {
    ...
    openNewExchange, xfDir, saveExchange,
