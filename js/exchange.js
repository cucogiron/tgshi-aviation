...
var Exports = (function() {
  ...
  // ---- Open export modal ----
  // preset: 'flights', 'fuel', 'schedule', 'maintenance', 'all', or undefined (= all)
  function openExportModal(preset) {
    ...
    var chkFlights = (!preset || preset === 'all' || preset === 'flights');
    var chkFuel = (!preset || preset === 'all' || preset === 'fuel');
    var chkSched = (!preset || preset === 'all' || preset === 'schedule');
    var chkMaint = (!preset || preset === 'all' || preset === 'maintenance');
    ...
    document.getElementById('book-form').innerHTML =
      ...
      + '<label style="font-size:12px;display:flex;align-items:center;gap:6px"><input type="checkbox" id="exp-flights"' + (chkFlights ? ' checked' : '') + '> Logbook (vuelos)</label>'
      + '<label style="font-size:12px;display:flex;align-items:center;gap:6px"><input type="checkbox" id="exp-fuel"' + (chkFuel ? ' checked' : '') + '> Combustible</label>'
      + '<label style="font-size:12px;display:flex;align-items:center;gap:6px"><input type="checkbox" id="exp-schedule"' + (chkSched ? ' checked' : '') + '> Agenda (reservas)</label>'
      + '<label style="font-size:12px;display:flex;align-items:center;gap:6px"><input type="checkbox" id="exp-maintenance"' + (chkMaint ? ' checked' : '') + '> Mantenimiento</label>'
      + '</div></div>'
