...
document.getElementById(‘edit-form-content’).innerHTML = ` <div class="fs"><label class="fl">Nombre del socio</label><input type="text" id="xp-name" placeholder="ej. Juan Pérez"></div> <div class="fs"><label class="fl">Aeronaves del socio (una por línea)</label> <textarea id="xp-planes" placeholder="TG-ABC&#10;TG-DEF" style="text-transform:uppercase;min-height:70px"></textarea> <div class="hint">Ingresa cada matrícula en una línea separada</div> </div> <div class="fs"><label class="fl">Tasa de intercambio (hrs suyas por 1 hr nuestra)</label><input type="number" id="xp-rate" value="1" step="0.1" inputmode="decimal"><div class="hint">1.0 = paridad, 1.5 = 1.5 hrs suyas por cada 1 hr nuestra</div></div> <div class="fs"><label class="fl">Notas</label><input type="text" id="xp-notes" placeholder="opcional"></div> <button class="btn" onclick="Exchange.addPartner()">Crear socio</button>`;
...
document.getElementById(‘edit-form-content’).innerHTML = ` <div class="fs"><label class="fl">Nombre</label><input type="text" id="xpe-name" value="${p.name}"></div> <div class="fs"><label class="fl">Aeronaves (una por línea)</label> <textarea id="xpe-planes" style="text-transform:uppercase;min-height:70px">${(p.planes || []).join('\n')}</textarea> <div class="hint">Ingresa cada matrícula en una línea separada</div> </div> <div class="fs"><label class="fl">Tasa de intercambio</label><input type="number" id="xpe-rate" value="${p.exchange_rate}" step="0.1" inputmode="decimal"></div> <div class="fs"><label class="fl">Notas</label><input type="text" id="xpe-notes" value="${p.notes || ''}"></div> <div style="display:flex;gap:8px"><button class="btn" onclick="Exchange.savePartner(${id})">Guardar</button><button class="btn" style="background:#8B1A1A" onclick="Exchange.deletePartner(${id})">Eliminar</button></div>`;
document.getElementById(‘edit-modal’).style.display = ‘flex’;
}
