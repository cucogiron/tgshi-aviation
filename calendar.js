...
const Calendar = (() => {
  ...
  function buildPlaneSelectors() {
    const planes = DB.planes.filter(p => p.active !== false);
    const html = planes.map(p =>
      `<div class="plane-chip${p.id === selPlane ? ' on' : ''}" onclick="Calendar.selectPlane('${p.id}',this)">${p.id}</div>`
    ).join('');
  ...
  function selectPlane(id, el) {
    selPlane = id;
    document.querySelectorAll('.plane-chip').forEach(c => c.classList.remove('on'));
  ...
  function buildCalendar() {
    ...
    const schedMap = {};
    (DB.schedule || []).filter(s => s.date.startsWith(monthStr) && s.plane_id === selPlane && s.status !== 'cancelled').forEach(s => {
      if (!schedMap[s.date]) schedMap[s.date] = [];
      schedMap[s.date].push(s);
    });

    let html = DAYS_ES.map(d => `<div class="cal-dh">${d}</div>`).join('');
  ...
  function buildDayDetail(ds) {
    const scheds = (DB.schedule || []).filter(s => s.date === ds && s.plane_id === selPlane && s.status !== 'cancelled')
    ...
    if (scheds.length === 0) {
      html += `<div class="empty" style="padding:20px">Sin reservas — disponible todo el día</div>`;
    } else {
      scheds.forEach(s => {
        ...
        html += `<div class="slot">
          ...
          <div class="slot-body">
            ...
            <div class="slot-meta"><span>${u.icon} ${s.booked_by}</span><span>🧑‍✈️ ${pilotDisplay}</span>${s.notes ? `<span>${s.notes}</span>` : ''}</div>
          </div>
  ...
  function openBooking(ds) {
    ...
    document.getElementById('book-form').innerHTML = `
      ...
      <div class="hint" style="text-align:center;margin-top:8px">Fernando confirmará y asignará piloto</div>`;
  ...
  async function submitBooking() {
    ...
    const existing = (DB.schedule || []).filter(s => s.date === date && s.plane_id === selPlane && s.status !== 'cancelled');
    const conflict = existing.find(s => start < s.end && end > s.start);
    ...
    DB.schedule.push({
      id, plane_id: selPlane, date, start, end,
      booked_by: App.currentUser(), pilot: null, pilot_roster_id: null,
      status: 'requested', flight_type: null, route, notes
    });

    closeBooking();
    const ok = await API.saveData();
    if (ok) { buildCalendar(); buildSchedPending(); }
    else { alert('Error guardando reserva'); }
  }

  // --- Confirmation modal ---
  function openConfirmModal(schedId) {
    const s = (DB.schedule || []).find(x => x.id === schedId);
    if (!s) return;
    cfmFlightType = null;

    document.getElementById('book-modal-title').textContent = 'Confirmar vuelo';
    const activePilots = (DB.pilots || []).filter(p => p.active !== false);
    ...
    const types = [
      { v: 'PERSONAL', icon: '✈', label: 'Personal', desc: 'Sin ingreso' },
      { v: 'STD', icon: '💼', label: 'Charter STD', desc: '$750/hr' },
      { v: 'FF', icon: '👥', label: 'Charter FF', desc: '$650/hr' },
      { v: 'MANTE', icon: '🔧', label: 'Mante', desc: 'Sin ingreso' }
    ];
    ...
    document.getElementById('book-form').innerHTML = `
      ...
      <div class="fs"><label class="fl">Tipo de vuelo</label>
        <div class="confirm-type-grid" id="cfm-type-grid">
          ${types.map(t => `<div class="confirm-type-card" data-t="${t.v}" onclick="Calendar.cfmTipo('${t.v}',this)"><div class="ti">${t.icon}</div><div class="tn">${t.label}</div><div class="td">${t.desc}</div></div>`).join('')}
        </div>
      </div>
  ...
  function cfmTipo(v, el) {
    cfmFlightType = v;
    document.querySelectorAll('#cfm-type-grid .confirm-type-card').forEach(c => c.classList.remove('on'));
    el.classList.add('on');
  }
  ...
  async function submitConfirmation(schedId) {
    ...
    const ok = await API.saveData();
    if (ok) { buildCalendar(); buildSchedPending(); }
    else { alert('Error confirmando vuelo'); }
  }
  ...
  function buildSchedPending() {
    const pends = (DB.schedule || []).filter(s => s.status === 'requested');
    ...
    sec.innerHTML = `<div class="pend-section"><div class="pend-hd"><span class="pend-ht">📅 Reservas pendientes</span><span class="pend-count">${pends.length}</span></div>
      ${pends.map(s => `<div class="pend-item"><div><div class="pend-info"><b>${s.route || 'Sin ruta'}</b> · ${s.plane_id} · ${s.start}-${s.end}</div><div class="pend-meta">${s.date} · ${App.getUser(s.booked_by).name}</div></div><div class="pend-actions"><button class="ver-btn" onclick="Calendar.openConfirmModal(${s.id})">✓</button><button class="rej-btn" onclick="Calendar.cancelSlot(${s.id})">✕</button></div></div>`).join('')}
    </div>`;
  }

  return {
    buildPlaneSelectors, selectPlane, buildCalendar, calNav, selectDay,
    openBooking, closeBooking, submitBooking,
    openConfirmModal, cfmTipo, submitConfirmation,
    cancelSlot, buildSchedPending
  };
})();
